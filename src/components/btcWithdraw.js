...
const BtcWithdrawList = () => { // Rename the function to start with an uppercase letter
  ...
  const fetchPendingBtcFundings = async () => {
    try {
      const response = await fetch('https://dripdash.onrender.com/api/getBtcWithdrawals');
  ...
  const updateUserBalance = async (transactionId, newStatus, userId, price_amount) => {
    try {
      const response = await fetch(`https://dripdash.onrender.com/api/updateUserWithdrawal/${transactionId}`, {
        method: 'PUT',
        headers: {
